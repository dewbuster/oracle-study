...
-- SCOTT에서 기본SELECT문을 익히기 위해 테이블 생성
-- 1) sql plus 실행해서 SYS 계정으로 접속
-- 2) c:\\oraclexe폴더 scott.sql 파일 검색
SELECT *
FROM tabs;
-- 오라클은 사용자 계정(SCOTT)을 생성하면 동일한 이름으로 DB객체를 생성한다
-- => 스키마(Schema)
DESC emp; -- 테이블의 구조 확인 쿼리
DESC dept;
DESC SALGRADE;
-- EMP 테이블에 사원 정보 조회
-- DQL : SELECT문
WITH        --1
SELECT *    --6     DISTINCT, ALL, AS 대문자 권장
FROM emp;   --2
WHERE       --3
GROUP BY    --4
HAVING      --5
...
-- SCOTT 소유 테이블 정보 조회
SELECT *
FROM dba_tables;  -- dba롤x 권한x
FROM user_tables; -- 내 소유
FROM all_tables;  -- 권한 있는 모든
FROM tabs;
-- DQL : SELECT문
SELECT *
FROM emp;
...
INSERT INTO insa (num, name, ssn, ibsaDate, city, tel, buseo, jikwi, basicPay, sudang) VALUES
  (1060, '김신애', '810809-2111111', '2001-10-10', '서울', '011-4151-4444', '개발부', 
   '사원', 900000 , 102000);
-- DML은 반드시 COMMIT, ROLLBACK을 해야한다
COMMIT;
